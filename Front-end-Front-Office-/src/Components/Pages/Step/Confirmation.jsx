...
if (stripePublicKey) {
  ...
  console.info("Stripe publishable key not set. Stripe payments will be disabled.");
}
...
// Helper function to calculate prices in INR
const calculatePrices = (reservation) => {
  if (!reservation) return { price: 0, formatted: formatCurrency(0) };

  const priceInINR = reservation.totalPrice || 0;
  
  return {
    price: priceInINR,
    formatted: formatCurrency(priceInINR)
  };
};

// Local Payment Component - India (original price in INR)
const RazorpayPayment = ({ reservation, onSuccess }) => {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const options = {
        key: process.env.REACT_APP_RAZORPAY_KEY_ID || 'rzp_test_yOMeMyaj2wlvTt',
        amount: amountInPaise,
        currency: 'INR',
        name: 'parkEz',
        description: `Parking Reservation - ${reservation.parkingId?.name || 'Parking'}`,
        ...
        prefill: {
          ...
          contact: reservation.userId?.phone || '',
        },
        theme: {
          color: '#2563eb',
        },
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      <div className="bg-blue-600 p-4">
        <h3 className="text-xl font-bold text-white flex items-center gap-2">
          <Flag className="w-5 h-5" /> Local Payment (India)
        </h3>
      </div>
      ...
      <div className="p-6">
        <div className="flex items-center justify-center p-6 bg-blue-50 rounded-lg mb-6">
          <div className="text-center">
            <svg className="mx-auto mb-3 h-12" viewBox="0 0 1896 401" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M382.571 232.762C382.571 267.191 354.572 295.19 320.143 295.19H0V170.333H320.143C354.572 170.333 382.571 198.333 382.571 232.762Z" fill="#072654"/>
              <path d="M382.571 170.333V107.619C382.571 73.1904 354.572 45.1904 320.143 45.1904H0V170.333H382.571Z" fill="#3395FF"/>
            </svg>
            <p className="text-gray-700 text-sm">
              Pay securely using Razorpay, India's most trusted payment gateway supporting UPI, Cards, Net Banking & Wallets.
            </p>
          </div>
        </div>
        ...
        <div className="flex justify-center">
          <button
            ...
            className={`bg-blue-600 text-white px-6 py-3 rounded-full font-semibold 
                    shadow-lg flex items-center space-x-2 hover:bg-blue-700 transition-all duration-300 
          ...
          >
            {loading ? (
              ...
              `Pay with Razorpay ${formatCurrency(totalPrice)}`
            )}
          </button>
        </div>
      </div>
...
// Payment Form Component - For International Users (prices treated as INR)
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="p-6">
        ...
        <div className="flex items-center justify-between bg-gray-50 p-4 rounded-lg mb-6">
          <span className="text-gray-700 font-medium">Total Amount:</span>
          <span className="text-xl font-bold text-blue-600">
            INR ₹{convertedPrice?.toFixed(2) || "0.00"}
          </span>
        </div>
        ...
        <div className="bg-blue-50 p-3 rounded-lg mb-6 text-sm">
          <p className="flex items-center text-blue-700">
            ...
            <span>
              Original price: INR ₹{reservation?.totalPrice?.toFixed(2) || "0.00"}
            </span>
          </p>
          <p className="text-blue-700 pl-6">
            Converted price: INR ₹{convertedPrice?.toFixed(2) || "0.00"}
          </p>
        </div>
        <div className="flex justify-center">
        <button
        ...
        >
          {processing ? (
            ...
            `Pay with Stripe INR ₹${convertedPrice?.toFixed(2) || "0.00"}`
          )}
        </button>
        </div>
      </form>
...
const PaymentSelection = ({ reservationId, reservation, onSuccess }) => {
  ...
  useEffect(() => {
    if (reservation?.paymentMethod === "fastag" || reservation?.fastagTagId) {
  ...
  const WalletPayment = ({ reservationId, onSuccess, autoPay = false }) => {
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const autoPayTriggered = React.useRef(false);
    ...
    const handleWalletPay = useCallback(async () => {
      ...
      try {
        ...
        onSuccess && onSuccess(reservationId);
      } catch (err) {
        setError(err.message);
      } finally {
        setLoading(false);
    ...
    useEffect(() => {
      ...
      handleWalletPay();
    }, [autoPay, handleWalletPay]);

    return (
      <div className="bg-white rounded-lg shadow-md overflow-hidden p-6">
        <div className="mb-4">
          <h4 className="text-lg font-semibold">Pay with Wallet</h4>
          <p className="text-sm text-gray-600">Available balance: INR ₹{(walletBalance||0).toFixed(2)}</p>
          {autoPay && (
            <p className="text-xs text-blue-600 mt-1">RFID payment detected, processing wallet debit automatically...</p>
          )}
        </div>
        {error && <div className="text-red-600 mb-3">{error}</div>}
        <button
          onClick={handleWalletPay}
          disabled={loading || (walletBalance || 0) < (originalPrice || 0)}
          className={`w-full py-3 rounded-lg font-semibold text-white ${
            loading || (walletBalance || 0) < (originalPrice || 0)
              ? 'bg-gray-400 cursor-not-allowed'
              : 'bg-green-600 hover:bg-green-700'
          }`}
        >
          {loading ? 'Processing...' : `Pay INR ₹${(originalPrice||0).toFixed(2)} from Wallet`}
        </button>
        {walletError && <div className="text-sm text-red-500 mt-2">{walletError}</div>}
      </div>
    );
  };
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            >
              <Flag className="w-8 h-8 text-green-600 mb-2" />
              <span className="font-medium">Wallet</span>
              <span className="text-sm text-gray-500">Use account wallet</span>
              <div className="mt-2 text-sm font-medium text-blue-600">
      ...
      {paymentMethod === "stripe" ? (
        stripePromise ? (
          ...
          <div className="p-4 text-sm text-red-600">
            Stripe is not configured (REACT_APP_STRIPE_PUBLIC_KEY missing). Please ask the
            administrator to set the publishable key to enable Stripe payments.
          </div>
        )
      ) : paymentMethod === 'wallet' ? (
        <WalletPayment
          reservationId={reservation._id}
          onSuccess={onSuccess}
          autoPay={reservation.paymentMethod === "fastag" || !!reservation.fastagTagId}
        />
      ) : (
        <RazorpayPayment reservation={reservation} onSuccess={onSuccess} />
      )}
    </div>
  );
};
...
const Confirmation = () => {
  ...
  // Add print handler
  // Note: print handler removed to silence ESLint unused-variable warnings.
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
              ...
              <div className="p-6">
                ...
                <DetailItem
                  ...
                  label="Start Time"
                  value={formatDate(reservation.startTime)}
                />

                <DetailItem
                  icon={<Calendar className="w-5 h-5 text-blue-600" />}
                  label="End Time"
                  value={formatDate(reservation.endTime)}
                />

                <div className="mt-6 pt-4 border-t border-gray-100">
                  <div className="flex justify-between items-center mb-4">
                    <span className="font-medium">Total Price (INR):</span>
                    <span className="text-xl font-bold text-blue-600">
