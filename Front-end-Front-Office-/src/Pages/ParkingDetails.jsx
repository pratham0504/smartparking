...
const ParkingDetails = ({ parkingData, isPopup }) => {
  ...
  useEffect(() => {
    ...
    if (parkingData) {
      ...
      const formattedData = {
        ...
        position: {
          lat: parseFloat(parkingData.position?.lat || parkingData.lat) || 19.0760,
          lng: parseFloat(parkingData.position?.lng || parkingData.lng) || 72.8777
        },
      ...
      axios.get(`${getBackendUrl()}/api/parkings/${id}`)
        .then(response => {
          ...
          const formattedData = {
            ...
            position: {
              lat: parseFloat(data.position?.lat || data.lat) || 19.0760,
              lng: parseFloat(data.position?.lng || data.lng) || 72.8777
            },
  ...
  useEffect(() => {
    if (isLoaded && mapContainer.current && parking && parking.position && 
        typeof parking.position.lng === 'number' && typeof parking.position.lat === 'number') {
    ...
    <div className="p-4 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
            <TitleBox icon="📡">Realtime Occupancy</TitleBox>
            <div className="p-4">
              <LiveParkingStatus
