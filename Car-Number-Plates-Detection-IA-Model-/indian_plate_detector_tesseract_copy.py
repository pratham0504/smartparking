...
class IndianPlateDetector:
    ...
    def format_indian_plate(self, text):
        ...
        # Indian plate patterns from reference
        patterns = [
            r'([A-Z]{2})(\d{1,2})([A-Z]{1,2})(\d{4})',  # MH05BB9140
            r'([A-Z]{2})(\d{2})([A-Z]{1,2})(\d{3,4})',  # Variations
        ]
        ...
        # If no pattern matches but we have text
        if len(text) >= 8 and len(text) <= 12:
            if text[:2].isalpha() and text[2:4].isdigit():
                ...
                if letter_end > 0:
                    series = remaining[:letter_end]
                    number = remaining[letter_end:]
                    return f"{state}-{district}-{series}-{number}"
        
        return text if len(text) >= 6 else None

    def normalize_plate_for_lookup(self, plate_text):
        """Return a normalized, DB-friendly variant for matching/lookups.

        Removes non-alphanumeric characters and uppercases the string so
        backends can compare on a canonical form (e.g. MH05BB9140).
        """
        if not plate_text:
            return None
        try:
    ...
    def process_image_array(self, img):
        ...
        try:
            ...
            # Save last OCR candidates for external lookup/matching
            try:
                self.last_ocr_candidates = unique_results
            except Exception:
                self.last_ocr_candidates = []
            ...
            for text in unique_results:
                ...
                if formatted and len(formatted) >= 6:
                    # Simple confidence based on text length and format
                    conf = 0.8 if '-' in formatted else 0.6
                    
                    # Bonus for Indian plate pattern
                    if re.match(r'[A-Z]{2}-\d{2}-[A-Z]{1,2}-\d{3,4}', formatted):
                        conf += 0.1
                    
                    if conf > best_confidence:
                        best_confidence = conf
                        best_result = formatted
                        logger.info(f"✓ Best result so far: {formatted} ({conf:.2f})")
...
def detect_plate():
    ...
    try:
        ...
        # Provide normalized and candidate forms to help downstream matching
        normalized = detector.normalize_plate_for_lookup(plate_text)
        candidates = getattr(detector, 'last_ocr_candidates', []) or []
